...
//        User principal = (User) session.getAttribute(Define.PRINCIPAL);
        int rowResultCount = vipReviewService.saveToVipReview(VipReviewSaveDTO, 1);
...
//        User principal = (User) session.getAttribute(Define.PRINCIPAL);
        int rowResultCount = vipReviewService.deleteToVipReview(VipReviewDeleteDTO, 1);
        return ResponseEntity.ok().body(ApiUtils.success(rowResultCount));
    }
...
//        User principal = (User) session.getAttribute(Define.PRINCIPAL);
        int rowResultCount = vipReviewService.updateToVipReview(VipReviewUpdateDTO, 1);
        return ResponseEntity.ok().body(ApiUtils.success(rowResultCount));
    }
